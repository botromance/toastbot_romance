/* Description:
   Romance novel plot generator - adapted from  toastlifeboat/lifebot 

 Author:
caughtinabreadromance channel */

const firstPerson = [
    'A viking who has been dead for 1,000 years',
    'A cyborg with a heart of gold',
    'A bloodthirsty sorcerer and his trash boyfriend',
...
let storypart = array[Math.floor(Math.random() * array.length)];
return storypart;
}


function toastbotRomance  ()   {
     let firstPers = random(firstPerson);
     let secondPers = random(secondPerson);
     let meetCute = random(romanceScenario);
     let setting = random(romanceSetting);
     let follows = random(whatFollows);
     console.log(`${firstPers} and ${secondPers} ${meetCute} ${setting}. ${follows}`);
      }
